...
public class DataFieldManager implements IDataFieldManager {
	...
	@Autowired
	private IDaoSupport<DataField> daoSupport;
	...
	/*
	 * (non-Javadoc)
	 * @see com.enation.app.cms.core.service.IDataFieldManager#getFieldByModel(int)
	 */
	@Override
	public List<DataField> getFieldByModelId(int modelId) {
		
		// 如果模型id有效
		if (modelId != 0) {
			List<DataField> list = this.getFieldByModelId(modelId);
			return list;
		}
		return null;
	}
	...
	/*
	 * (non-Javadoc)
	 * @see com.enation.app.cms.core.service.IDataFieldManager#getFieldByModelNames(java.lang.String)
	 */
	@Override
	public List<DataField> getFieldByModelNames(String englishName) {
		
		//如果名称有效
		if (englishName != null && !"".equals(englishName)) {
			DataModel dataModel = this.dataModelManager.get(englishName);
			
			// 如果存在这个模型
			if (dataModel != null) {
				List<DataField> list = this.getFieldListByModelId(dataModel.getModel_id());
				return list;
			}
		}
		return null;
	 ...
	 * @return
	 */
	private  List<DataField> getFieldListByModelId(int modelId) {
		String sql = "SELECT * FROM es_data_field WHERE model_id = ?";
