...
public class GoodsApiController {
	...
	public JsonResult list(Integer page, Integer cat, String sort,
			Integer brand, Integer seckill, String keyword) {
		if (page == null || page <= 0) {
			page = 1;
		}
	...
	public JsonResult filter() {
		...
		for (int i = 0; i < types.length; i++) {
			if (map.containsKey(types[i])) {
				...
				for (Object obj : valueList) {
					...
					filterValueList
							.add(new ApiFilterValue(selector.getName(),
									UrlUtils.getParameter(selector.getUrl(),
											types[i])));
				}
		...
		if (map.containsKey("prop")) {
			...
			for (Object key : propMap.keySet()) {
				...
				for (Object obj : valueList) {
					SearchSelector selector = (SearchSelector) obj;
					filterValueList.add(new ApiFilterValue(selector.getName(),
							UrlUtils.getParameter(selector.getUrl(), "prop")));
				}
				apiFilter.setValueList(filterValueList);
				filterList.add(apiFilter);
			}
	...
	public JsonResult detail(Integer goodsid, Integer act_id,Integer groupbuy_id) {
		...
		int goodCommentCount = apiCommentManager.getCommentsCount(goodsid, 2, 3);
		...
		if (groupbuy_id != null && groupbuy_id > 0) {
			Map groupbuyGoods = apiGoodsManager.getGroupbuyGoods(goodsid,
					groupbuy_id);
		...
		// 促销信息
		Activity activity = storeActivityManager.getCurrentAct(NumberUtils.toInt(productMap.get("store_id").toString(), 0));
		if(activity != null){
			if(activity.getRange_type().intValue() == 1 || (activity.getRange_type().intValue() == 2 && storeActivityManager.checkGoodsAct(goodsid, activity.getActivity_id())==1)){
				...
				//判断是否包含优惠券
				if(map.containsKey("is_send_bonus") && NumberUtils.toInt(map.get("is_send_bonus").toString(), 0) == 1){
	...
	public JsonResult spec(Integer goodsid) {
		...
		data.put("have_spec",
				goods.get("have_spec") == null ? 0 : goods.get("have_spec"));// 是否有规格
	...
	public JsonResult listbytag(Integer tagid, Integer page) {
		if (page == null || page <= 0)
			page = 1;
		int pageSize = 10;
		List<Map> goodsList = apiGoodsManager
				.listByTag(tagid, page, pageSize);
	...
	public void qrcode(Integer goods_id, HttpServletResponse response)
			throws Exception {
		...
		// 将图片输出给浏览器
		BufferedImage image = QrCodeUtils.create("javashop://" + goods_id,
