...
function formatShow(value, row, index){
	...
	if(value == 0){
		val = '<p id="floorstatus'+row.id+'" style="display:inline;margin:0px 10px">已显示</p>'
			+'<input class="stop button" type="button" value="停用" floorid="'+row.id+'" id=floorimg'+row.id+' onclick="changeshow('+row.id+')" showvalue="0" changevalue="1" >';
	}else if(value == 1){
		val = '<p id="floorstatus'+row.id+'" style="display:inline;margin:0px 10px">未显示</p>'
			+'<input class="start button" type="button" value="启用" floorid="'+row.id+'" id=floorimg'+row.id+' onclick="changeshow('+row.id+')" showvalue="1" changevalue="0" >';
	}
	return val;
}
