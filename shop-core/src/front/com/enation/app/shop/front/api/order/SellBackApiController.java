...
public class SellBackApiController {
	 ...
	 * @param return_type 【必填】退货类型数组 0：普通退货，1：套餐商品主商品退货，2：套餐商品捆绑商品部分退货
	 * @return
	 */
	@ResponseBody
	@RequestMapping(value="/apply")
	public JsonResult apply(Integer[] goodsId,Integer[] goodsNum, Integer[] payNum, Integer[] productId, Double[] price
			,Integer[] item_id, Integer[] return_type, SellBackList sellBack){
		try {
			...
			if (goodsId != null) {
				for(int i=0;i<goodsId.length;i++){
					...
					if (return_type != null && return_type[i] != 2) {// 添加为空验证  add by liuyulei 2016-10-09
					...
					sellBackGoods.setReturn_type(return_type == null ? 0 : return_type[i]); // 添加为空验证  add by liuyulei 2016-10-09
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="/add-refund", produces = MediaType.APPLICATION_JSON_VALUE)
	public JsonResult addRefund(SellBackList sellBack){
		try{
			//记录会员信息
			Member member =  UserConext.getCurrentMember();
