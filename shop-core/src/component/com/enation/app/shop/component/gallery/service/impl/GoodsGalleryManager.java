...
public class GoodsGalleryManager implements IGoodsGalleryManager {
	...
	@Autowired
	private IDaoSupport<GoodsGallery> daoSupport;

	//分布式文件管理
	private static IClusterFileManager clusterFileManager;

	private static IClusterFileManager getClusterFileManager(){
		if(ClusterSetting.getFdfs_open()==1){
			return clusterFileManager != null ? clusterFileManager : (IClusterFileManager)SpringContextHolder.getBean("clusterFileManager");
		}
		return null;
	...
	public List<GoodsGallery> list(int goods_id) {
		...
		for (GoodsGallery gallery : result) {
			if (!StringUtil.isEmpty(gallery.getOriginal()))
				gallery.setOriginal(StaticResourcesUtil.convertToUrl(gallery.getOriginal()));
	...
	@Override
	public List<GoodsGallery> listWithOutDefault(int goods_id) {
		List<GoodsGallery> result = this.daoSupport.queryForList("select gg.*,g.params from es_goods_gallery gg left join es_goods g on gg.goods_id=g.goods_id where gg.goods_id = ? and gg.isdefault = 0 ORDER BY gg.sort", GoodsGallery.class, goods_id);
		for (GoodsGallery gallery : result) {
			if (!StringUtil.isEmpty(gallery.getOriginal()))
				gallery.setOriginal(StaticResourcesUtil.convertToUrl(gallery.getOriginal()));
			if (!StringUtil.isEmpty(gallery.getBig()))
