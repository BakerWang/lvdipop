...
public class OrderApplyShortMsgPlugin extends AutoRegisterPlugin implements IShortMessageEvent{
	...
	private ShortMsg getPendingSettlement(){
		StringBuffer sql=new StringBuffer("select count(0) from es_order where is_cancel=0 and ( ( payment_type!='cod' and  status="+OrderStatus.ORDER_CONFIRM +") ");//非货到付款的，未付款状态的可以结算
	...
	private ShortMsg getToBeShipped(){
		...
		if(count>0){
			...
			msg.setContent("有"+count +"个待发货订单申请需要完成");
			msg.setTitle("待发货订单");
			msg.setTarget("ajax");
			return msg;
		}
	...
	private ShortMsg getRefundApply(){
		...
		if(EopSetting.PRODUCT.equals("b2b2c")){
			sql= "select count(0) from es_sellback_list where type=? AND tradestatus=? AND store_id=1 order by id desc";
		}else{
			sql= "select count(0) from es_sellback_list where type=? AND tradestatus=? order by id desc";
		} 
		int count = this.daoSupport.queryForInt(sql,1,SellBackStatus.wait.getValue());
		if(count>0){
			ShortMsg msg  = new ShortMsg();
	...
	private ShortMsg getSellBackApply(){
		String sql= "select count(0) from es_sellback_list where type=? AND tradestatus=? order by id desc";
		int count = this.daoSupport.queryForInt(sql,2,SellBackStatus.apply.getValue());
		if(count>0){
			ShortMsg msg  = new ShortMsg();
	...
	private ShortMsg getRefundList(){
		...
		if(count>0){
			ShortMsg msg  = new ShortMsg();
			msg.setUrl("/shop/admin/order-report/refund-list.do");
