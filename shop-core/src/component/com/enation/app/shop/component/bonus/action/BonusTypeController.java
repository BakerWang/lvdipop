...
public class BonusTypeController extends GridController {
	...
	public JsonResult saveAdd(BonusType bonusType, String useTimeStart, String useTimeEnd, String sendTimeStart,
		...
		if (StringUtil.isEmpty(bonusType.getRecognition())) {
			return JsonResultUtil.getErrorJson("请输入优惠卷识别码");
		}
		...
		try {
			...
			return JsonResultUtil.getSuccessJson("保存优惠卷类型成功");
		} catch (Throwable e) {
			this.logger.error("保存优惠卷类型出错", e);
			return JsonResultUtil.getErrorJson("保存优惠卷类型出错" + e.getMessage());
		}
		...
		try {
			...
			if (this.bonusTypeManager.checkForActivity(type_id) == 1) {
				...
				if (this.bonusTypeManager.checkIsSend(type_id) == 1) {
					...
					return JsonResultUtil.getSuccessJson("删除优惠卷类型成功");
				}
			}
			
		} catch (Throwable e) {
			this.logger.error("删除优惠卷类型出错", e);
			return JsonResultUtil.getErrorJson("删除优惠卷类型出错"+e.getMessage());
		}
