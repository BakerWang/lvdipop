...
public class ReturnsOrderManager implements IReturnsOrderManager {
	...
	public String getSnByOrderSn(String orderSn) {
		return (String)this.daoSupport.queryForObject("select goodsns from es_returns_order where ordersn = ?", new StringMapper(), orderSn);
	}
	...
	public Double queryItemPrice(Integer orderid,Integer state) {
		Double temp=(Double)this.daoSupport.queryForObject("SELECT sum(price) as price FROM es_order_items  where order_id = ? and state= ?",new DoubleMapper(),orderid,state);
		return temp;
	}
