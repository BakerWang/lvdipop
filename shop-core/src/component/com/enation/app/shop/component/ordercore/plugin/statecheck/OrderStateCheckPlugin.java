...
public class OrderStateCheckPlugin extends AutoRegisterPlugin implements IEveryDayExecuteEvent {
	...
	private void checkcmpl(){
		...
		String sql = "select order_id from es_order where ? >=  signing_time+? and  signing_time is not null and   signing_time>0  and status!=?";
		List list = this.daoSupport.queryForList(sql,unix_timestamp,time,OrderStatus.ORDER_COMPLETE);
	...
	private void checkRog(){
		//查询已发货 的订单
		String sql = "select d.* from es_order o ,es_delivery d where o.order_id=d.order_id and (o.status=? and o.payment_type!='cod')";
		
		List<Delivery>  deliList  =  this.daoSupport.queryForList(sql,Delivery.class,OrderStatus.ORDER_SHIP) ;
	...
	private void checkCancel(){
		...
		String sql="SELECT order_id from es_order  WHERE disabled=0 AND create_time+?<? AND status=?  ";
		List<Map> list= daoSupport.queryForList(sql,time,DateUtil.getDateline(),OrderStatus.ORDER_NOT_PAY);
		for (Map map:list) {
