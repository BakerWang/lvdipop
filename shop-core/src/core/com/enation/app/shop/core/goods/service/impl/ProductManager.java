...
public class ProductManager implements IProductManager {
	...
	@Autowired
	private IDaoSupport<Product> daoSupport;
		...
		for(Product pro:prolist){
			...
			for(int i=size;i>=0;i--){
				...
				if(  pro.getProduct_id().intValue()  ==   productid  ){ 
					...
					pro.setSpecs((String)gs.get("spec_value"));
					pro.addSpec(spec);
				}
	...
	@Override
	public String getSpecValByProductId(Integer productId) {
		String sql = "SELECT spec_value FROM es_goods_spec gs INNER JOIN es_spec_values sv ON gs.spec_value_id = sv.spec_value_id WHERE gs.product_id = " + productId;
