...
public class GoodsCatManager  implements IGoodsCatManager {
	...
	@Autowired
	private IDaoSupport<Cat> daoSupport;
...
//				}
	...
	public void update(Cat cat) {
		...
		if (cat.getParent_id() != null && cat.getParent_id().intValue() != 0) {
			...
			String sql = "select * from es_goods_cat where cat_id=?";
			Cat parent = daoSupport.queryForObject(sql, Cat.class, cat.getParent_id());
...
//				}
	...
	@Override
	public List<Map> listCatByLevel(int level) {
		List<Map> levelCatList = new ArrayList<Map>();
		String sql = "select DISTINCT c.cat_id,c.name,c.cat_path,c.parent_id from es_goods g left join es_goods_cat c "
				+ "on g.cat_id = c.cat_id where length(cat_path) - length(replace(cat_path,'|','') ) = ? order by c.cat_order asc";
		levelCatList = daoSupport.queryForList(sql, level);
		return levelCatList;
	}
