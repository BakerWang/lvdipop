...
public class MemberPointManger extends BaseSupport implements IMemberPointManger {
	...
	public void add(Member member, int point, String itemname,Integer relatedId, int mp) {
		...
		this.baseDaoSupport.execute("update es_member set point=point+?,mp=mp+? where member_id=?", point, mp, member.getMember_id()); 
	...
	public void useMarketPoint(int memberid,int point,String reson,Integer relatedId){
		...
		pointHistoryManager.addPointHistory(pointHistory);
		this.baseDaoSupport.execute("update es_member set mp=mp-? where member_id=?", point,memberid); 
	}
