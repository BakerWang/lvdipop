...
public class MemberAddressManager implements IMemberAddressManager {
	...
	@Autowired
	private IDaoSupport<MemberAddress> daoSupport;
	@Autowired
	...
	@Override
	public int addAddress(MemberAddress address, Integer member_id) {
		MemberAddress defAddr = this.getMemberDefault(member_id);
		if (defAddr == null) {
			address.setDef_addr(1);
		} else {
			if (address.getDef_addr() == 1) {
				daoSupport.execute("update es_member_address set def_addr = 0 where member_id = ?", member_id);
			}
		}
