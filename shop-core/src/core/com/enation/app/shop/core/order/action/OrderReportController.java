...
public class OrderReportController extends GridController {
	...
	public ModelAndView returned(Integer sid){
		...
		SellBackList sellBackList = this.sellBackManager.get(sid);//退货详细
	...
	public GridJsonResult refundListJson(){
		return JsonResultUtil.getGridJson(refundManager.refundList(this.getPage(), this.getPageSize()));
	}
	...
	public ModelAndView refundDetail(Integer id){
		...
		if(sellback_id!=null){
			SellBackList sellBackList = this.sellBackManager.get(sellback_id);
