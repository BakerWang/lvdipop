...
public class OrderReportManager implements IOrderReportManager {
	...
	public double getPayMoney(Integer paymentId) {
		return (Double) this.daoSupport.queryForObject(
				"select sum(pay_money) from es_payment_detail where payment_id=?",
				new DoubleMapper(), paymentId);
	}
	 ...
	 */
	private boolean isAllStorage(Integer id){
	...
	private void confirmPartStorage(int id) {
		this.daoSupport.execute("update es_sellback_list set tradestatus= 5  where id=?", id);
