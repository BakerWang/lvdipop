...
//			throw new RuntimeException("创建订单失败，购物车为空");
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public Order add(Order order, List<CartItem> itemList, Integer member_id)  {
//		String opname = "游客";
		if (order == null)
			throw new RuntimeException("error: order is null");
		/************************** 用户信息 ****************************/
...
//		}
		...
		order.setGoods_num(itemNumTotal);
		/** 检测商品库存 End **/
//		this.orderPluginBundle.onBeforeCreate(order, itemList, null);
		this.daoSupport.insert("es_order", order);
		Integer orderId = this.daoSupport.getLastId("es_order");
		order.setOrder_id(orderId);
		this.saveGoodsItem(itemList, order);
		...
		orderManager.addLog(orderId, "订单创建", opname);
		order.setOrder_id(orderId);
		this.orderPluginBundle.onAfterCreate(order, itemList, null);
		//SSO 接入计算优惠券
		// 下单则自动改为已确认
		if (!order.getIsCod()) {
			this.confirmOrder(orderId);
	...
	public boolean pay(Integer paymentId,Integer orderId,double payMoney,String userName) {
		...
		double paidmoney=this.orderReportManager.getPayMoney(paymentId);//其它金额中已结算的金额
			...
			if(CurrencyUtil.add(payMoney, paidmoney)<payment.getMoney()){
				...
				//更新订单的已付金额
				this.daoSupport.execute("update es_order set paymoney=?,status=?,pay_status=? where order_id=?",payMoney,OrderStatus.ORDER_PAY,OrderStatus.PAY_YES,order.getOrder_id());
	...
	public Order payConfirm(int orderId){
		...
		System.out.println("订单物流状态" + order.getShip_status());
	...
	public void rogConfirm(int orderId,Integer op_id,String op_name,String sign_name,Long sign_time){
		...
		if(order.getPay_status() == OrderStatus.PAY_YES){
			orderstatus= OrderStatus.ORDER_COMPLETE;
			sql="update es_order set status=" + orderstatus + ",ship_status=" + OrderStatus.SHIP_ROG + ",the_sign='"+ sign_name + "',signing_time=" + sign_time+ " ,complete_time="+DateUtil.getDateline()+",sale_cmpl=1  where order_id=" + orderId;
		}
		...
		if(order.getMember_id()!=null){
			...
			for (Map map : itemList) {
				this.daoSupport.execute(
								"INSERT INTO es_member_order_item(member_id,goods_id,order_id,item_id,commented,comment_time) VALUES(?,?,?,?,0,0)",
								order.getMember_id(), 
