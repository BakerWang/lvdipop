...
public class GoodsController extends GridController {
	...
	public ModelAndView list() {
		...
		view.addObject("tagList", tagManager.list());
	...
	public GridJsonResult listJson(Integer catid,Integer stype,String keyword,String name,String sn) {
		...
		webpage =this.goodsManager.searchGoods(goodsMap, this.getPage(), this.getPageSize(), null,"buy_count",this.getOrder());
	...
	@ResponseBody
	@RequestMapping(value = "/recommondgood")
	public JsonResult RecommondGoods(){
		try {
			Integer goods_id = Integer.valueOf(ThreadContextHolder.getHttpRequest().getParameter("goods_id"));
			this.goodsManager.recommondGoods(goods_id, 1);
			return JsonResultUtil.getSuccessJson("推荐商品成功");
		} catch (Exception e) {
			logger.error("推荐商品失败", e);
			return JsonResultUtil.getErrorJson("推荐商品失败");
		}
	}
	
	@ResponseBody
	@RequestMapping(value = "/notrecommondgood")
	public JsonResult notrecommondGoods(){
		try {
			Integer goods_id = Integer.valueOf(ThreadContextHolder.getHttpRequest().getParameter("goods_id"));
			this.goodsManager.recommondGoods(goods_id, 0);
			return JsonResultUtil.getSuccessJson("取消推荐成功");
		} catch (Exception e) {
			logger.error("取消推荐失败", e);
			return JsonResultUtil.getErrorJson("取消推荐失败");
		}
	}
