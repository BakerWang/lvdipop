...
public class DlyTypeManager implements IDlyTypeManager {
	...
	@Autowired
	private IDaoSupport<DlyType> daoSupport;
	...
	public int getDlyTotal() {
		...
		String sql = "";
		if (EopSetting.PRODUCT.equals("b2b2c")) {
			sql = "select count(0) from es_dly_type dt,es_store_template st where st.id=dt.template_id and st.store_id=? and def_temp=1 order by type_id";
