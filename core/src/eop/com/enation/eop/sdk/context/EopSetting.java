...
public class EopSetting {
	...
	public static double POINTRATE = 100d;
	
	public static double PAYRATE = 0.1d;
	
	
	public static String MESSAGE_HTTPURL = "http://api.xianguo.greenlandjs.com/v1/mobile/send/sms";
	
	public static String MESSAGE_TOKENKEY = "lvdisendcontent";
 ...
 //		InputStream in  =  FileUtil.getResourceAsStream("../../config/eop.properties"); 
		...
		CRMURL = props.getProperty("crmurl");//调用crm接口配置url
		POINTRATE = Double.valueOf(props.getProperty("pointrate"));//积分与钱的比率
		PAYRATE = Double.valueOf(props.getProperty("payrate"));//可使用积分多少的比率
		
		MESSAGE_HTTPURL = String.valueOf(props.getProperty("httpUrl")); //短信发送的URL
		
		MESSAGE_TOKENKEY = String.valueOf(props.getProperty("TOKEN_KEY"));//短信token_key
