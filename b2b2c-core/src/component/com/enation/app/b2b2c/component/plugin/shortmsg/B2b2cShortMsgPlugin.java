...
public class B2b2cShortMsgPlugin extends AutoRegisterPlugin implements IShortMessageEvent {
	...
	private ShortMsg getPayConfirmMessage(){
		...
		String sql = "select count(0) num  from es_order where disabled = 0 AND store_id=?  AND is_cancel=0 AND parent_id is NOT NULL  and ( ( payment_type!='cod' and  status=1) or ( payment_type='cod' and  (status=5 or status=6 )  ) )";
		int count = this.daoSupport.queryForInt(sql,ShopApp.self_storeid);
		if(count>0){
			ShortMsg msg  = new ShortMsg();
			msg.setUrl("/b2b2c/admin/store-order/not-pay-order.do");
			msg.setTitle("待结算订单");
			msg.setTarget("ajax");
	...
	private ShortMsg getSellBackApply(){
		String sql="select count(0) from es_sellback_list WHERE store_id=? AND type=? AND tradestatus=? order by id desc";
		int count = this.daoSupport.queryForInt(sql,ShopApp.self_storeid,2,SellBackStatus.apply.getValue());
		if(count>0){
			ShortMsg msg  = new ShortMsg();
			msg.setUrl("/b2b2c/admin/store-sell-back/refund-list.do?type=2");
			msg.setContent("自营店有"+count +"个退货申请需要完成");
			msg.setTitle("退货申请");
			msg.setTarget("ajax");
			return msg;
		}
