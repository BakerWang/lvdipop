...
public class StoreSellBackManager implements IStoreSellBackManager {
	...
	public void refund(Integer id, Double alltotal_pay) {
		 ...
		 * 记录订单日志
		 */
		orderManager.addLog(order.getOrder_id(), "等待退款，退款金额："+alltotal_pay,storeManager.getStore().getStore_name());
	...
	public void authRetund(Integer id,Integer status,String seller_remark) {
		...
		Integer type = (Integer) sellBackMap.get("type");
		
		//日志信息 add_by DMRain 2016-7-21
		String log = "";
		
		//获取退货单中的赠品id add_by DMRain 2016-7-21
		Integer gift_id = (Integer) sellBackMap.get("gift_id");
		...
		if(sellBackMap!=null){
			if(status==1){
				if(UserConext.getCurrentMember()!=null){
					...
					orderManager.addLog(Integer.parseInt(sellBackMap.get("orderid").toString()), log, storeManager.getStore().getStore_name());
					 ...
					 */
					if (gift_id != null) {
						this.orderGiftManager.updateGiftStatus(gift_id, Integer.parseInt(sellBackMap.get("orderid").toString()), 2);
					}
				}
			}else{
				//判断单据的类型 1：退款单，2：退货单
				...
				//修改订单状态为可申请退款
				this.daoSupport.execute("update es_order set status=? where order_id=?",OrderStatus.ORDER_COMPLETE,Integer.parseInt(sellBackMap.get("orderid").toString()));
				
				//根据订单id获取订单商品货物集合 add_by DMRain 2016-7-26
				List<OrderItem> itemList = this.orderManager.listGoodsItems(Integer.parseInt(sellBackMap.get("orderid").toString()));
				 ...
				 */
				if (gift_id != null) {
					this.orderGiftManager.updateGiftStatus(gift_id, Integer.parseInt(sellBackMap.get("orderid").toString()), 0);
				}
			}
		}
