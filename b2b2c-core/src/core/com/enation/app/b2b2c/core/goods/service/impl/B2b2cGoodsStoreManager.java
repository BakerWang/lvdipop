...
public class B2b2cGoodsStoreManager implements IB2b2cGoodsStoreManager {
	...
	public Page listSelfStore(Map map, int page, int pageSize, String sortField, String sortType) {
		...
		sql.append("select g.goods_id,g.sn,g.name,g.store,c.name cname,g.enable_store from es_goods g,es_goods_cat c where g.cat_id=c.cat_id and g.disabled=0 and g.store_id="+ShopApp.self_storeid);
