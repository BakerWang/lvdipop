...
public class StoreOrderManager implements IStoreOrderManager{
	...
	/*
	 * (non-Javadoc)
	 * @see com.enation.app.b2b2c.core.order.service.IStoreOrderManager#pageBuyerOrders(int, int, java.lang.String, java.lang.String)
	 */
	@Override
	public Page pageBuyerOrders(int pageNo, int pageSize, String status, String keyword){
		...
		if(!StringUtil.isEmpty(keyword)){
			sql.append(" AND order_id in (SELECT i.order_id FROM es_order_items i INNER JOIN es_order o ON i.order_id=o.order_id WHERE o.member_id=?"
					+ " AND (i.name like ? OR o.sn LIKE ? OR (CASE i.goods_type when 0 then '普通商品' WHEN 1 then '服务商品' end ) like ?))");
			
			argsList.add(member.getMember_id()); 
			argsList.add("%"+keyword+"%"); //将关键字做为name参数查询 压入参数list
			argsList.add("%"+keyword+"%"); //将关键字做为sn参数查询 压入参数list
			argsList.add("%"+keyword+"%"); //将关键字做为goods_type参数查询 压入参数list
	...
	public Page pageChildOrders(int pageNo, int pageSize, String status,
			String keyword) {
		StoreMember member = storeMemberManager.getStoreMember();
		StringBuffer sql =new StringBuffer("select * from es_order where member_id = '" + member.getMember_id() + "' and disabled=0");
		if(!StringUtil.isEmpty(status)){
		...
		if(!StringUtil.isEmpty(keyword)){
			sql.append(" AND order_id in (SELECT i.order_id FROM es_order_items i LEFT JOIN es_order o ON i.order_id=o.order_id WHERE o.member_id='"+member.getMember_id()+"' AND i.name like '%" + keyword + "%')");
		}
	...
	/*
	 * (non-Javadoc)
	 * @see com.enation.app.b2b2c.core.order.service.IStoreOrderManager#orderStatusNum(java.lang.Integer)
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see com.enation.app.b2b2c.core.order.service.IStoreOrderManager#getStoreGoodsNum(int)
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see com.enation.app.b2b2c.core.order.service.IStoreOrderManager#saveShipNo(java.lang.Integer[], java.lang.Integer, java.lang.String, java.lang.String)
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see com.enation.app.b2b2c.core.order.service.IStoreOrderManager#censusState()
	 */
	@Override
...
//		sql+=" or (  status!="+OrderStatus.ORDER_NOT_PAY+"  and  pay_status!="+OrderStatus.PAY_NO +")" ; //此语句会影响待结算订单的查询，暂时先注释掉 add by DMRain 2016-7-14
	...
	/*
	 * (non-Javadoc)
	 * @see com.enation.app.b2b2c.core.order.service.IStoreOrderManager#getStoreSellbackOrder()
	 */
	@Override
	public int getStoreSellbackOrder() {
		StoreMember member = storeMemberManager.getStoreMember();
		String sql = "select count(0) from es_sellback_list where store_id = ?";
		return this.daoSupport.queryForInt(sql, member.getStore_id());
	}
	
	/*
	 * (non-Javadoc)
	 * @see com.enation.app.b2b2c.core.order.service.IStoreOrderManager#getCancelApplicationList(java.lang.Integer, java.lang.Integer)
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see com.enation.app.b2b2c.core.order.service.IStoreOrderManager#cancelApplicationList(java.lang.Integer, java.lang.Integer, java.lang.Integer)
	 */
	@Override
	...
	private String  createTempSql(Map map,String sortField,String sortType){
		...
		Integer status =  map.get("status")==null||StringUtil.isEmpty(map.get("status").toString()) ? null:Integer.parseInt(map.get("status").toString());
		
		String ship_name = (String) map.get("ship_name");
		
		Integer paystatus =  map.get("paystatus")==null||StringUtil.isEmpty(map.get("paystatus").toString()) ? null:Integer.parseInt(map.get("paystatus").toString());
		
		Integer shipstatus = map.get("shipstatus")==null||StringUtil.isEmpty(map.get("shipstatus").toString()) ? null:Integer.parseInt(map.get("shipstatus").toString());
		
		Integer shipping_type =  map.get("shipping_type")==null||StringUtil.isEmpty(map.get("shipping_type").toString()) ? null:Integer.parseInt(map.get("shipping_type").toString());
		
		Integer payment_id =  map.get("payment_id")==null||StringUtil.isEmpty(map.get("payment_id").toString()) ? null: Integer.parseInt(map.get("payment_id").toString());
		
		Integer depotid = map.get("depotid")==null||StringUtil.isEmpty(map.get("depotid").toString()) ? null: Integer.parseInt( map.get("depotid").toString());
		
		String complete =map.get("complete")==null||StringUtil.isEmpty(map.get("complete").toString()) ?null: (String)map.get("complete").toString();
		
		String store_name= map.get("store_name")==null||StringUtil.isEmpty(map.get("store_name").toString()) ?null:map.get("store_name").toString();
		
		Integer store_id= map.get("store_id")==null||StringUtil.isEmpty(map.get("store_id").toString()) ? null:Integer.parseInt(map.get("store_id").toString());
		
		String parent_sn=map.get("parent_sn")==null||StringUtil.isEmpty(map.get("parent_sn").toString()) ?null:(String) map.get("parent_sn").toString();
		
		StringBuffer sql =new StringBuffer();
 
		sql.append("select * from es_order o join es_member e on o.member_id = e.member_id where o.disabled=0 and o.parent_id is NOT NULL "); //只查询出子订单
